...
class WaymoChunkDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        super().__init__(
            dataset_cfg=dataset_cfg, class_names=class_names, training=training, root_path=root_path, logger=logger
        )
        self.sub_dataset = WaymoDataset(
            dataset_cfg, class_names, training=training, root_path=root_path, logger=logger
        )

        assert getattr(self.dataset_cfg, "chunk_len")
        self.chunk_length = dataset_cfg.chunk_len

        self.seq_name_to_indices = {}
        for index in range(len(self.sub_dataset)):
            seq_name = self.sub_dataset.infos[index]['point_cloud']['lidar_sequence']
            frame_id = self.sub_dataset.infos[index]['point_cloud']["sample_idx"]
            if seq_name not in self.seq_name_to_indices:
                self.seq_name_to_indices[seq_name] = []
            self.seq_name_to_indices[seq_name].append(
                (frame_id, index)
            )
        self.length = 0
        backoff = self.chunk_length
        self.items = []
        self.per_seq_indices = {}
        for seq_name in self.seq_name_to_indices:
            self.seq_name_to_indices[seq_name].sort(key=lambda x: x[0])
            start_ids = self.seq_name_to_indices[seq_name]
            per_seq_indices_list = []
            for i in range(0, len(start_ids) - backoff + 1):
                self.items.append(
                    start_ids[i:i + backoff]
                )
                per_seq_indices_list.append(len(self.items) - 1)
            self.per_seq_indices[seq_name] = per_seq_indices_list
    ...
    def __getitem__(self, index):
        ...
        for i, item in enumerate(items):
            sub_index = item[1]
            batch_dict = self.sub_dataset[sub_index]
            out_batch_dict[f"item_{i}"] = batch_dict
...
class WaymoDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        data_dict["vehicle_to_world"] = pose_cur
        data_dict["sequence_name"] = sequence_name
        return data_dict
